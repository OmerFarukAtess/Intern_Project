...
def get_customer_and_card_info(customer_no):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM CustomersLimit WHERE CustomerNo=?", customer_no)
        ...
        cursor.execute("SELECT CardNo, CardDailyLimit, CardRemainingDailyLimit FROM CardsLimit WHERE CustomerNo=?", customer_no)
...
def update_customer_info(new_customer_daily_limit, new_customer_remaing_daily_limit, customer_no):
    conn_str = r'DRIVER={SQL Server};SERVER=DESKTOP-7PPE35F\MSSQLSERVER1;DATABASE=VakıfBank;Trusted_Connection=yes;'
    try:
        conn = pyodbc.connect(conn_str)
        cursor = conn.cursor()
        cursor.execute(
            ...
            new_customer_daily_limit, new_customer_remaing_daily_limit, customer_no)
        conn.commit()
...
def update_customer_card_info(new_card_daily_limit, new_card_kalan_daily_limit, card_no, customer_no):
    ...
    try:
        ...
        cursor.execute(
            ...
            new_card_daily_limit, new_card_kalan_daily_limit, customer_no, card_no)
        conn.commit()
        return True
    except Exception as e:
