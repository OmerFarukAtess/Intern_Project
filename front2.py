...
def musteri_bilgileri_goster(data):
    ...
    if float(data["Customer"]["CustomerRemainingDailyLimit"]) < 0:
        st.write("Müşteri Kalan Günlük Limit: 0.00")
    else :
        st.write(f"Müşteri Kalan Günlük Limit: {data['Customer']['CustomerRemainingDailyLimit']}")

    st.subheader("Kart Bilgileri")
    for kart in data['Cards']:
        ...
        if float(kart['CardDailyLimit']) > float(data['Customer']['CustomerDailyLimit']):
            ...
            st.write(f"Kart Günlük Limit: {kart['CardDailyLimit']}")
            if float(kart["CardRemainingDailyLimit"]) < 0:
                st.write("Kart Kalan Günlük Limit: 0.00")
            else:
                st.write(f"Kart Kalan Günlük Limit: {kart['CardRemainingDailyLimit']}")
        st.write("---")
    ...
    if st.session_state.show_update_customer_limit_input:
        ...
        if st.button("Müşteri Limitini Kaydet"):
            ...
            if response.status_code == 200:
                ...
                st.session_state.current_customer_data['Customer']['CustomerRemainingDailyLimit'] = response.json()['CustomerRemainingDailyLimit']
                musteri_bilgileri_goster(st.session_state.current_customer_data)
                ...
                if response.status_code == 200:
                    ...
                    for i, card in enumerate(st.session_state.current_customer_data['Cards']):
                        if card['CardNo'] == selected_card_no:
                            ...
                            st.session_state.current_customer_data['Cards'][i]['CardRemainingDailyLimit'] = response.json()['CardRemainingDailyLimit']
                            break
                    musteri_bilgileri_goster(st.session_state.current_customer_data)
        ...
        if not kart_numaralari:
            ...
            if st.button("Para Çek"):
                ...
                if response.status_code == 200:
                    ...
                    st.session_state.current_customer_data['Customer'] = response.json()['Customer']
                    for i, card in enumerate(st.session_state.current_customer_data['Cards']):
                        if card['CardNo'] == response.json()['Cards']['CardNo']:
                            st.session_state.current_customer_data['Cards'][i] = response.json()['Cards']
                            break
