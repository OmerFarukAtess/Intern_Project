...
from dataBaseConnection import get_customer_and_card_info
...
def customer_info():
    ...
    customer_and_card_info = get_customer_and_card_info(customer_no)
...
def customer_limit():
    ...
    customer_object = Customer(
        ...
        customer_and_card_info["Customer"]["CustomerRemainingDailyLimit"]
    )
    ...
    if customer_object.update_customer_limit(new_limit):
        ...
        current_customer_info = get_customer_and_card_info(customer_no)
        return jsonify({
            "durum": "Müşteri günlük limiti başarıyla güncellendi.",
            "CustomerDailyLimit": current_customer_info["Customer"]["CustomerDailyLimit"],
            "CustomerRemainingDailyLimit": current_customer_info["Customer"]["CustomerRemainingDailyLimit"]
        })
    else:
...
def card_limit():
    ...
    card_object = Customer(
        ...
        selected_card_data["CardRemainingDailyLimit"]
    )
    ...
    if card_object.update_card_limit(new_limit):
        ...
        return jsonify({
            ...
            "CardRemainingDailyLimit": current_customer_card_info["CardRemainingDailyLimit"]
        })
    else:
...
def withdraw_money():
    ...
    durum = withdrawn_object.withdraw_money(withdrawn_amount)
